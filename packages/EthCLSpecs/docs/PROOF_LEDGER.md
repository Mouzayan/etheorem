...
what is open. `getPtc`, `shouldExtendPayload`, and
...
| Function | Location | Property | Status | Tracking |
| --- | --- | --- | --- | --- |
| `shouldExtendPayload` | `Heze/ForkChoice.lean:320-342` | Under successful preliminary lookup and slot checks, a verified payload with a recorded `false` inclusion-list satisfaction verdict is rejected by the FOCIL gate, with the pure runner state unchanged. This is not complete correctness of `shouldExtendPayload`: the later Gloas accept and reject paths and the missing-record `assert` branch stay out of scope, and the payload/verdict pairing tracked under `onExecutionPayloadEnvelope` remains open | in progress | #63, `Proofs/Heze/ShouldExtendPayload.lean` |
| `recordPayloadInclusionListSatisfaction` | `Heze/ForkChoice.lean:406-418` | Proved for successful runs: if `state.slot` is nonzero and timely transaction collection for the previous slot succeeds (using the default `onlyTimely := true`), the recorder stores `isInclusionListSatisfied payload ilTxs` at `root`. It leaves the runner state exactly as transaction collection returned it. The recorded result may be either `true` or `false`. Still open: the slot-zero case and transaction-collection failures, including an empty committee or a missing timeliness entry. These error paths must also be covered before the theorem can be tagged `characterizes` and the function can move beyond the `touched` tier | in progress | #82, `Proofs/Heze/RecordPayloadInclusionListSatisfaction.lean` |
| `onExecutionPayloadEnvelope` | `Heze/ForkChoice.lean:426-448` | After a successful envelope acceptance, `payloads[root]` and `payloadInclusionListSatisfaction[root]` are written together for the same `root`, so a verified payload is paired with some recorded satisfaction verdict. Still open: formally prove this same-root pairing and show that looking up the recorded result can return `false`, as required by `shouldExtendPayload_run_eq_false_of_recorded_unsatisfied`. The generic `FcMap` interface provides no rule connecting `insert` with `lookup`. The existing `treeMap` lookup theorem also cannot be used because it requires an unavailable `TransCmp Root` instance | proposed |  |
