...
| `recordPayloadInclusionListSatisfaction` | `Heze/ForkChoice.lean:406-418` | Under nonzero `state.slot` and a successful `getInclusionListTransactions` run at `state.slot - 1` (default `onlyTimely = true`), the pure runner `ForkChoiceStoreRun (Store map)` returns the explicit store with only `payloadInclusionListSatisfaction` updated by `FcMap.insert` of `isInclusionListSatisfied payload ilTxs` at `root`, and leaves the runner state unchanged. Both Boolean verdicts are this write. The slot-0 underflow and the transaction-collection rejects stay outside the claim | proved | `Proofs/Heze/RecordPayloadInclusionListSatisfaction.lean` |
