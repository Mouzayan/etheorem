...
| Function | Location | Property | Status | Tracking |
| --- | --- | --- | --- | --- |
| `shouldExtendPayload` | `Heze/ForkChoice.lean:320-342` | Under successful preliminary lookup and slot checks, a verified payload with a recorded `false` inclusion-list satisfaction verdict is rejected by the FOCIL gate, with the pure runner state unchanged. This is not complete correctness of `shouldExtendPayload`: the later Gloas accept and reject paths and the missing-record `assert` branch stay out of scope, and the `onExecutionPayloadEnvelope` pairing row stays open | in progress | #63, `Proofs/Heze/ShouldExtendPayload.lean` |
| `recordPayloadInclusionListSatisfaction` | `Heze/ForkChoice.lean:406-418` | Landed so far: under nonzero `state.slot` and a successful transaction collection (default `onlyTimely = true` at `state.slot - 1`), the recorder returns the update and threads through the runner state that collection produced. The returned store has only `payloadInclusionListSatisfaction` updated by `FcMap.insert` of `isInclusionListSatisfied payload ilTxs` at `root`. Both Boolean verdicts are this write. Open: the slot-0 `checkedSub` underflow and the transaction-collection rejects, including the empty-committee and missing-timeliness-key paths. A complete result covering those branches is what would carry a `characterizes` tag and move the function past the touched tier | in progress | #82, `Proofs/Heze/RecordPayloadInclusionListSatisfaction.lean` |
| `onExecutionPayloadEnvelope` | `Heze/ForkChoice.lean:426-448` | After a successful envelope acceptance, `payloads[root]` and `payloadInclusionListSatisfaction[root]` are written together for the same `root`, so a verified payload is paired with some recorded satisfaction verdict. Open: same-root payload/verdict structural pairing, and a `false` lookup that would discharge `shouldExtendPayload_run_eq_false_of_recorded_unsatisfied`. Generic `[FcMap map]` has no insert/lookup law, and the tested `treeMap` lookup theorem currently lacks `TransCmp` for `Root` | proposed |  |
