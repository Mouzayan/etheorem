...
| `recordPayloadInclusionListSatisfaction` | `Heze/ForkChoice.lean:406-418` | Under nonzero `state.slot` and a successful transaction collection that preserves the runner state (default `onlyTimely = true` at `state.slot - 1`), the recorder returns the update with that state unchanged. The returned store has only `payloadInclusionListSatisfaction` updated by `FcMap.insert` of `isInclusionListSatisfied payload ilTxs` at `root`. Both Boolean verdicts are this write. The slot-0 underflow and the transaction-collection rejects stay outside the claim | proved | `Proofs/Heze/RecordPayloadInclusionListSatisfaction.lean` |
