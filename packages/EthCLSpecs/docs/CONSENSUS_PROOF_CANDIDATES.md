...
| `getIndexedPayloadAttestation`     | `Gloas/Operations.lean:378-384`      | Preserves `pa.data` and `pa.signature`, and produces the sorted multiset of PTC seats selected by `pa.aggregationBits`. The proof requires `Array.qsort` sortedness and permutation lemmas not currently available in core/Std |
| `isValidIndexedPayloadAttestation` | `Gloas/Operations.lean:389-400`      | **Proved**, see `EthCLSpecs/Proofs/IsValidIndexedPayloadAttestation.lean`. Returns true exactly when the attesting indices are non-empty, adjacent-nondecreasing (duplicates permitted), all within the validator registry, and the configured `[CryptoBackend]` accepts the exact aggregate-verification call |
| `processPayloadAttestation`        | `Gloas/Operations.lean:402-410`      | Running `processPayloadAttestation` does not modify `State`: it only reads the input state and evaluates three assertions. A concrete run-level theorem should show that both success and rejection preserve the state component |
