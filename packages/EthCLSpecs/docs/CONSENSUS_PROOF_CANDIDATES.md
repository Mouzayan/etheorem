...
| `getExpectedWithdrawals`         | `Gloas/Withdrawals.lean:173-181`    | The withdrawals returned by its four phases combined never exceed `MAX_WITHDRAWALS_PER_PAYLOAD`                                                   |
| `initiateBuilderExit` | `Gloas/Operations.lean:90-93` | **Proved**, see `EthCLSpecs/Proofs/InitiateBuilderExit.lean`. Its exact in-range/out-of-range effect on the builder registry is characterized; no-wrap is conditional for an arbitrary `Config` and proved unconditionally for both shipped Gloas preset/config pairs |
| `processBuilderExitRequest` | `Gloas/Operations.lean:196-206` | On its successful builder-exit branch, the index supplied to `initiateBuilderExit` is in range; under either shipped Gloas preset/config pair, the selected builder receives the intended non-wrapping future `withdrawableEpoch`. All non-matching or ineligible branches leave the builder registry unchanged |
| `getPtc`                         | `Gloas/Operations.lean:391-408`     | **Proved**, see `EthCLSpecs/Proofs/GetPtc.lean`. Its computed offset into `ptcWindow` stays in range under the two guarded call patterns covered here: `data.slot + 1 == state.slot` and the fork-choice replay callers' `slot == curSlot` |
| `canBuilderCoverBid`             | `Gloas/Operations.lean:462-465`     | Primary builder-solvency predicate; the natural theorem is that accepting a bid never leaves the builder insolvent                                |
...
| `isValidIndexedPayloadAttestation` | `Gloas/Operations.lean:424-435`      | Returns true only when the attesting indices are non-empty, sorted, within the validator set, and the aggregate signature verifies |
| `processExecutionPayloadBid`       | `Gloas/Operations.lean:494-528`      | The self-build and builder-bid paths it chooses between are mutually exclusive and jointly exhaustive                              |
| `applyParentExecutionPayload`      | `Gloas/Operations.lean:533-557`      | Exactly one of settle-current, settle-previous, or evict fires, so a payment is never settled twice                                |
...
| `stateTransition`   | `Gloas/Transition.lean:109-119` | Top-level state-transition correctness; composes `processSlots`, `processBlock`, and the root check into the canonical transition                                                                  |
