...
| `initiateBuilderExit` | `Gloas/Operations.lean:90-93` | **In review**, see `EthCLSpecs/Proofs/InitiateBuilderExit.lean`. Its exact in-range/out-of-range effect on the builder registry is characterized; no-wrap is conditional for an arbitrary `Config` and proved unconditionally for both shipped Gloas preset/config pairs |
| `processBuilderExitRequest` | `Gloas/Operations.lean:192-206` | On its successful builder-exit branch, the index supplied to `initiateBuilderExit` is in range; under either shipped Gloas preset/config pair, the selected builder receives the intended non-wrapping future `withdrawableEpoch`. All non-matching or ineligible branches leave the builder registry unchanged |
